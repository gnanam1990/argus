...
| The in-sandbox control server is RCE-shaped | `guestd` binds localhost by default; a routable bind requires `ARGUS_GUEST_TOKEN` bearer auth. Transport is rate-limited and per-command audited; run it over TLS. |
